...
class Price:
	...
	def coinList(self):
		return Url.get_url(self.__coinlisturl)
	...
	def coinSnapshot(self, from_curr, to_curr):
		return Url.get_url(self.__coinsnapshoturl + 'fsym=' + from_curr.upper() + '&tsym=' + to_curr.upper())

	def coinSnapshotFullById(self, coin_id):
		return Url.get_url(self.__coinsnapshotfull + 'id=' + str(coin_id))
	...
	def __get_price(self, baseurl, from_curr, to_curr, e=None, extraParams=None, sign=False,
		...
		if len(args) >= 2:
			return Url.get_url(baseurl + '&'.join(args))
		else:
...
class Social:
	def __init__(self):
		self.__socialstatsurl = 'https://www.cryptocompare.com/api/data/socialstats/?'

	def socialStats(self, cryptoid):
		if isinstance(cryptoid, str):
			return Url.get_url(self.__socialstatsurl+'id='+cryptoid)
		else:
			raise ValueError('Must have cryptoid argument of type str')

class News:
	def __init__(self):
		self.__newsurl = 'https://min-api.cryptocompare.com/data/news/'

	def getNews(self):
		return Url.get_url(self.__newsurl)

	def getNewsProviders(self):
		return Url.get_url(self.__newsurl + 'providers')
